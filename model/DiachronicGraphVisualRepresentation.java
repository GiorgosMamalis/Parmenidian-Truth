...
public class DiachronicGraphVisualRepresentation {
	...
	public DiachronicGraphVisualRepresentation(DiachronicGraph p, String in, String tf, int et,int mode,double frameX,double frameY,double scaleX,double scaleY,double centerX,double centerY) {		
		...
		universalFrame =new Dimension(parent.getSizeOfNodesList()*26, parent.getSizeOfNodesList()*26);
		...
		if(mode==1){
			for (int i = 0; i < parent.getSizeOfNodesList(); ++i) {
				layout.setLocation(parent.getKeyOfNode(parent.getNode(i)),parent.getCoordsOfNode(parent.getNode(i)));
	...
	public void saveVertexCoordinates(String projectIni) throws IOException {
		...
		//save sthn mnhmh
		for (int i = 0; i <parent.getSizeOfNodesList(); ++i)
			parent.setCoordsOfNode(layout.transform(parent.getKeyOfNode(parent.getNode(i))), i);
	...
	private boolean isLineOK(String line) {
		return (line.contains("Project Name") || line.contains("sql@") ||line.contains("transition@")||line.contains("output@"));
	}
